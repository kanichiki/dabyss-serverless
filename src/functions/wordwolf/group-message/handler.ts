...
const replyRollCallEnd = async (group: dabyss.Group, wordWolf: wordwolf.WordWolf, replyToken: string): Promise<void> => {
...
const replyWolfNumberChosen = async (wordWolf: wordwolf.WordWolf, wolfNumber: number, replyToken: string): Promise<void> => {
    const promises: Promise<void>[] = [];
...
const replyLunaticNumberChosen = async (wordWolf: wordwolf.WordWolf, lunaticNumber: number, replyToken: string): Promise<void> => {
    const promises: Promise<void>[] = [];
...
const replyConfirm = async (wordWolf: wordwolf.WordWolf, replyToken: string): Promise<void> => {
    ...
    await dabyss.replyMessage(replyToken, await replyMessage.main(userNumber, depth, wolfNumber, lunaticNumber, timerString));
    return;
};
...
const replyConfirmYes = async (wordWolf: wordwolf.WordWolf, replyToken: string): Promise<void> => {
    ...
    for (let i = 0; i < userIds.length; i++) {
        ...
        try {
            ...
            promises.push(dabyss.pushMessage(userIds[i], await pushMessage.main(displayNames[i], userWords[i], isLunatic[i])));
        } catch (err) {
            console.error(err);
        }
    }
...
const replyAnnounceResult = async (wordWolf: wordwolf.WordWolf, replyToken: string): Promise<void> => {
    ...
    promises.push(dabyss.replyMessage(replyToken,
