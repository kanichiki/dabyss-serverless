...
const replyVoteSuccess = async (wordWolf: wordwolf.WordWolf, votedUserIndex: number, userIndex: number, replyToken: string): Promise<void> => {
    ...
    if (isVoteCompleted) {
        ...
        if (!multipleMostVotedUserExists) {
            ...
            promises.push(dabyss.replyMessage(
                replyToken,
            ...
            if (!isRevoting) {
                ...
                const shuffleMostVotedUserIndexes: number[] = await dabyss.getRandomIndexes(mostVotedUserIndexes, mostVotedUserIndexes.length);
                ...
                promises.push(dabyss.replyMessage(replyToken, await replyMessage.main(voterDisplayName, shuffleMostVotedUserIndexes, displayNames)));
            } else {
