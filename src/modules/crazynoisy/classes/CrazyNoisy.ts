...
export class CrazyNoisy extends dabyss.Game {
	...
	async updatePositions() {
		...
		for (const k in positionNumbers) {
	...
	async getNotBrainwashedNumber() {
		let res = 0;
		for (const state of this.brainwashStatus) {
			if (!state) {
				res++;
			}
		}
	...
	async getWinnerIndexes() {
		...
		for (let i = 0; i < this.positions.length; i++) {
			if (this.winner == "guru") {
				...
				// 市民陣営勝利なら
				if (
					this.positions[i] == this.positionNames.detective ||
					this.positions[i] == this.positionNames.citizen ||
					this.positions[i] == this.positionNames.sp
				) {
					res.push(i);
				}
			}
	...
	async putAction() {
		...
		for (let i = 0; i < userNumber; i++) {
			if (
				this.positions[i] == this.positionNames.guru ||
				(this.positions[i] == this.positionNames.detective && !this.brainwashStatus[i])
			) {
				status[i] = false;
			} else {
				status[i] = true;
